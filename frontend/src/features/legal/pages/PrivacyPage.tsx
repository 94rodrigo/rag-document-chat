...
export function PrivacyPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-16">
      <h1 className="font-display text-4xl text-text-primary mb-2">{t('legal.privacy.title')}</h1>
      <p className="text-xs text-text-tertiary mb-8">{t('legal.lastUpdated', { date: lastUpdated })}</p>

      <p className="text-sm text-text-secondary leading-relaxed mb-10">{t('legal.privacy.intro')}</p>
      ...
      <p className="text-sm text-text-secondary mt-12 pt-8 border-t border-border-subtle">
        {t('legal.privacy.contactPrompt')}{' '}
        <Link to="/contact" className="text-accent hover:underline">{t('legal.privacy.contactLink')}</Link>
      </p>
    </div>
  )
}
