...
import { Trash2, TriangleAlert, FileText, Loader2 } from 'lucide-react'
...
export function DeleteDocumentDialog({ document, open, onOpenChange }: DeleteDocumentDialogProps) {
  ...
  const isPending = deleteDoc.isPending || confirmed
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription className="mt-3">
            {t('documents.deleteDialog.body')}{' '}
            <span className="text-text-primary font-medium">{t('documents.deleteDialog.bodyEmphasis')}</span>
          </DialogDescription>
        </DialogHeader>
